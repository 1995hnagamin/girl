...
bool
does_contain_alias(const std::string &line, const std::string &alias) {
  ...
  boost::algorithm::split(
      aliases, aliasline, boost::algorithm::is_any_of(","));
...
bool
is_section_title(const std::string &line) {
  return line.length() > 1 && line[0] == '#' && line[1] != '#';
}

boost::optional<Location>
find_section(const boost::filesystem::path &filepath,
  ...
  std::string row;
  int row_number = 1;
  bool is_previous_section =
      false; // whether previous line is section title ("# title...")
...
boost::optional<Location>
find_glossary(const boost::filesystem::path &glospath,
              const std::string &target) {
  namespace fs = boost::filesystem;
  fs::path glossary(glospath.string());
  for (fs::recursive_directory_iterator iter =
           fs::recursive_directory_iterator(glossary);
       iter != fs::end(iter);
       iter++) {
...
search_libpaths(const std::string &glossary,
...
boost::optional<Location>
find_library(const std::string &libpath, const std::string &target) {
  namespace fs = boost::filesystem;
...
boost::optional<Location>
find_libraries(const std::vector<std::string> &libpaths,
               const std::string &target) {
  for (std::string libpath : libpaths) {
...
boost::optional<std::string>
get_env(std::string key) {
  return get_env(key.c_str());
}

void
less(const Location &l) {
...
boost::optional<Location>
find_target(const boost::optional<std::string> &glossary,
            const std::vector<std::string> &libpaths,
            const std::string &target) {
  if (!glossary) {
    return find_libraries(libpaths, target);
  }
  boost::optional<boost::filesystem::path> glospath(
      search_libpaths(*glossary, libpaths));
  if (!glospath) {
    throw;
    // return boost::none;
  }
...
std::string
concat_arguments(const std::vector<std::string> &args) {
  return std::accumulate(args.begin(),
                         args.end(),
                         std::string(),
...
main(int argc, char **argv) {
  try {
    ...
    std::string target(
        concat_arguments(vm["query"].as<std::vector<std::string>>()));
    boost::optional<std::string> glossary(
        assoc<std::string>("glossary", vm));
    boost::optional<Location> location(
        find_target(glossary, getEnvs("GIRLPATH"), target));
    if (location) {
      less(*location);
    } else {
